...
This repo is the source for that image, published to `ghcr.io/getloom/keycloak-dev`
so any `getloom/*` project can reuse it without checking out `loom-app` (or this repo).
...
uses the `compose.yaml` in this repo, which pulls `ghcr.io/getloom/keycloak-dev:main`
and starts it on `:8080` with the same defaults as the table above. Copy or adapt this
file into a consuming project (as `loom-app` does) to run it alongside your app.
